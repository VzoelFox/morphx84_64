...
handle_structure() {
    ...
    if [[ "$type" == "scope_end" ]]; then
        ...
        if [[ "$scope" == "STRUKTUR" ]]; then
             ...
             # Constants are usually global, but let's prefix them if inside a Unit
             local const_name="${POP_ID}_SIZE"
             if [[ -n "$CURRENT_UNIT" ]]; then const_name="${CURRENT_UNIT}_${const_name}"; fi

             # We can't use compile_line directly because it parses args again.
             # Manually set LABEL
             if [[ $PASS -eq 1 ]]; then
...
compile_line() {
    ...
    # --- META DIRECTIVES (Generated by Phase 0) ---
    ...
    if [[ "$mnemonic" == "prop" ]]; then
        ...
        # Struct properties are scoped to struct, but struct name is already scoped.
        if [[ -n "$CURRENT_UNIT" ]]; then const_name="${CURRENT_UNIT}_${const_name}"; fi
